...
def return_sse_stock(keyword):
    ...
    com_code_list = []
    code_list = sse_stock.replace("_t.push", "")\
                         .replace("\n", "") \
                         .replace("(", "[")\
                         .replace(")", "]")\
                         .split(";")
    ...
    for code in code_list:
        ...
        if len(temp) > 0:
            ...
            if re.findall(find_stock, c) or re.findall(find_stock, n1) or re.findall(find_stock, n2):
                com_code_list.append({"market": "SSE", "code": c, "name": n1.replace(" ", "")})
...
def return_szse_stock(keyword):
    ...
    try:
        ...
        if data:
            for d in data:
                code_list.append({"market": "SZSE", "code": d["code"], "name": d["name"].replace(" ", "")})
    except Exception as ex:
        print(f"exception: {ex}")
    ...
    try:
        ...
        for stock in stocks:
            if stock["s"][0] == "0":
                ...
                code_list.append({"market": market, "code": code, "name": name})
    except Exception as ex:
        print(f"exception: {ex}")
